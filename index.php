...
if(!isset($_GET['hdKey']) && !isset($_GET['pid']) && !isset($_GET['id']) && !isset($_GET['key']) && !isset($_GET['ch']) && !isset($_GET['lice']) && !isset($_GET['ca']) && !isset($_GET['caId']) && !isset($_GET['catchupPid'])){
  ...
  for($i=0;$i<count($list);$i++){
    echo "<tr><td><a href='$path".$list[$i]['id']."' target='_blank'>".$list[$i]['name']."</a> ";
    if($list[$i]['fhd']!=""){
      $licenseData = $list[$i]['fhd'];
      $licenseData = explode("@",$licenseData);
      $licenseId = $licenseData[1];
      echo "| <a href='$path"."FHD/".$list[$i]['link']."/$licenseId' target='_blank'>Full HD</a> ";
      if($list[$i]['catchup'])
        echo "| <a href='$path"."Catchup/".$list[$i]['id']."/0' target='_blank'>Catchup</a></td></tr>";
      else
        echo "</td></tr>";
    }
    else{
  ...
  $channelUrl = str_replace("http://","",$channelUrl);
